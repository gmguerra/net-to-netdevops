...
Technically, Azure resources can be exposed to the internet, so creating a VPN is not needed. But what if exposing via internet is not _okay_? 

Azure VPN gateway Service used to send encrypted traffic between a VNet and on-premises network over the public internet, or VNet to VNet over Micosoft network. Uses a VNet gateway called VPN Gateway, and multiple connections can be created to the same VPN gateway. When multiple, all VPN tunnels share the available bandwidth. 
...
## Plan a VPN Gateway:
Factors to consider:
- Min subnet range for a Gateway Subnet: /29
- Recommended subnet range for a Gateway Subnet: /27. If an expressroute is added, /27 is mandatory.  
- Throughput
...
- VPN Gateway type
- VPN Gateway SKU: different SKUs with different limitations in connections/tunnels, throughput, routing support, redundancy and VMs inside VNet. Cannot convert from a SKU to another, would need to delete the resource and recreate it. . 
	- Basic SKU: super restrictive, not recommended. Policy-based (static) only. 1 tunnel available. Cannot coexist with ExpressRoute nor P2S VPN. Generation1 only. Route-based available, but limited. 
	- Any other SKU: route based (dynamic). Multiple tunnels. ExpressRoute and P2S VPN co-existence. BGP optionally. Active-Active possible. Traffic will be send to the tunnel before encrypting, so that's why multiple tunnels with different setups are available. Generation one and two. 
